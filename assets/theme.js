...
class DetailsMega extends DetailsDropdown {
  ...
  async transitionIn() {
    ...
    // Read computed CSS variables from header section
    const computedStyle = getComputedStyle(document.documentElement);
    const durationValue = computedStyle
      .getPropertyValue("--mega-menu-duration")
      .trim();
    const staggerValue = computedStyle
      .getPropertyValue("--mega-menu-stagger-delay")
      .trim();

    // Convert CSS values to milliseconds
    const duration = parseFloat(durationValue) || 0; // Duration is in seconds
    const staggerDelay = parseFloat(staggerValue) || 0; // Stagger is in milliseconds

    const motionReduced = theme.config.motionReduced;
    const animationsEnabled = duration > 0 && !motionReduced;

    const delay = animationsEnabled ? staggerDelay : 0;
    setTimeout(() => {
    ...
    return Motion.animate(
      ...
      { duration: animationsEnabled ? duration : 0, easing: [0.7, 0, 0.2, 1] }
    ).finished;
  }
...
class MotionList extends HTMLElement {
  ...
  setupHoverObserver() {
    // Add debounced hover state management to prevent conflicts
    this.hoverTimeout = null;
    this.addEventListener("mouseenter", (e) => {
      if (e.target.closest("[data-card-hover]")) {
        clearTimeout(this.hoverTimeout);
        this.hoverTimeout = setTimeout(() => {
          this.classList.add("has-hovered-card");
        }, 50);
      }
    });
    ...
    this.addEventListener("mouseleave", (e) => {
      if (!this.contains(e.relatedTarget)) {
        clearTimeout(this.hoverTimeout);
        this.classList.remove("has-hovered-card");
      }
    });
  }

  getAnimationParams() {
    // Read CSS custom properties for better integration
    const computedStyle = getComputedStyle(document.documentElement);
    const cardDuration =
      parseFloat(computedStyle.getPropertyValue("--card-animation-duration")) ||
      0.5;
    const staggerDelay =
      parseFloat(computedStyle.getPropertyValue("--card-stagger-delay")) || 0.1;

    return {
      distance: this.isMobile ? 30 : 50,
      duration: this.motionReduced ? 0 : this.isMobile ? 0.3 : cardDuration,
      staggerDelay: this.motionReduced
        ? 0
        : this.isMobile
        ? 0.05
        : staggerDelay,
  ...
  unload() {
    ...
    // Add animate-ready class for CSS targeting
  ...
  load() {
    ...
    // Enhanced animation with better card coordination
    const animation = Motion.animate(
      visibleItems,
      { y: [distance, 0], opacity: [0, 1], visibility: ["hidden", "visible"] },
      {
        duration: duration,
        delay: Motion.stagger(staggerDelay),
        easing: [0.3, 1, 0.3, 1],
      }
    );

    // Add animate-on-hover class after reveal
    animation.finished.then(() => {
      visibleItems.forEach((item) => {
        item.classList.add("animate-on-hover");
      });
    });
    ...
    Motion.animate(
      inVisibleItems,
      { y: [distance, 0], opacity: [0, 1], visibility: ["hidden", "visible"] },
      { duration: duration, easing: [0.3, 1, 0.3, 1] }
    );
  }
...
class MenuToggle extends HTMLButtonElement {
  constructor() {
    super();
...
class FiltersToggle extends HTMLButtonElement {
  constructor() {
    super();
